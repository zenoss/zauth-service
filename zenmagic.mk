...
ifndef CHECK_TOOLS
    ...
    CHECK_TOOLS += $(SORT) $(TAR) $(TEE)   $(TOUCH) $(XARGS) $(PIP)
    CHECK_TOOLS += $(EASY_INSTALL) $(PYTHON)
...
ifeq "$(REQUIRES_PYTHON)" "1"
    ...
    ifndef CHECK_TOOLS_VERSION_BRAND
        CHECK_TOOLS_VERSION_BRAND := pip:$(REQD_PIP_MIN_VER): python:$(REQD_PYTHON_MIN_VER):
    endif
...
#------------------------------------------------------------
# Invoke python <arg>.
quiet_cmd_PYTHON = PYTHON $2 $3 $4 $5 $6 $7 $8
      cmd_PYTHON = $(PYTHON) $2 $3 $4 $5 $6 $7 $8

#------------------------------------------------------------
# Invoke python pip <arg>.
quiet_cmd_PIP = PIP $2 $3 $4 $5
      cmd_PIP = $(PIP) $2 $3 $4 $5

#------------------------------------------------------------
# Invoke python easy_install <arg>.
quiet_cmd_EASY_INSTALL = EASY_INSTALL $2 $3 $4 $5
      cmd_EASY_INSTALL = $(EASY_INSTALL) $2 $3 $4 $5
 
#----------------------------------------------------------------------------
# Remove a file.
quiet_cmd_RM = RM $2 $3
      cmd_RM = $(RM) "$(strip $2)" "$(strip $3)"

#------------------------------------------------------------
# Copy
quiet_cmd_CP = CP $2 $3 $4
      cmd_CP = $(CP) $2 $3 $4
